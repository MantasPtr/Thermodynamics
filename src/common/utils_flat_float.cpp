...
float *empty_partial_matrix(int dim_x, int dim_y)
{
    // printf("EMPTY PARTIAL MATRIX = %d %d\n", dim_x, dim_y);
    // Innitiate
    float *matrix = new float[dim_x * dim_y];
    // Clean matrix
...
float *generate_part_of_the_matrix(int matrix_dimention, float max_value, int row_start, int row_end, int column_start, int column_end)
{
    ...
    float *matrix = empty_partial_matrix(row_end - row_start, column_end - column_start);
    // printf("FILLING WAVES\n");
    int NUMBER_OF_WAVES = 5;

    for (int i = 0; i < matrix_dimention; i++)
    {
        float value = abs(sin((float)i / matrix_dimention * PI * NUMBER_OF_WAVES)) * max_value;
        if (column_start == 0)
            matrix[i] = value;
    ...
    //     {
    //         matrix[i * matrix_dimention + j] = (float)max_value / ((matrix_dimention - 1) * 2) * (i + j);
    //     }
    // }
